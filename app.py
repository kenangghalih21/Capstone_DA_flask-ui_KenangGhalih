...
def index():
    ...
    ## Bar Plot
    cat_order = df2.groupby('Category').agg({'App' : 'count'}).rename({'Category':'Total'}, axis=1).sort_values('App', ascending=False).reset_index().head()
    ...
    title = "\tProportion for Installs/Downloaded of 'Content Rating'\n on 'SOCIAL' Category".expandtabs()
